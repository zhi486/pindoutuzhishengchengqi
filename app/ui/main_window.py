...
class MainWindow(ctk.CTk):
    ...
    def __init__(self):
        ...
        left.grid_rowconfigure(0, weight=0)   # 上传
        left.grid_rowconfigure(1, weight=1)   # 预览
        ...
        # --- 右侧: 控制 + 图例，整体可滚动 ---
        right_scroll = ctk.CTkScrollableFrame(main, fg_color="transparent")
        ...
        self.control_panel.pack(fill="x", pady=(0, 2))

        self.color_legend = ColorLegendPanel(right_scroll)
        self.color_legend.pack(fill="both", expand=True)
    ...
    def _build_header(self):
        ...
        ctk.CTkLabel(title_row, text="拼豆图纸生成器",
                     font=ctk.CTkFont(family="Microsoft YaHei", size=17, weight="bold"),
                     text_color="#f7f3ec").pack(side="left")

        ctk.CTkLabel(title_row, text="图片转拼豆图纸 · 自动匹配色号清单",
                     font=ctk.CTkFont(family="Microsoft YaHei", size=11),
                     text_color="#a2988a").pack(side="left", padx=12)

        ctk.CTkLabel(title_row, text="桌面版",
                     font=ctk.CTkFont(family="Microsoft YaHei", size=10),
                     text_color="#cfc6b8").pack(side="right")

        # 3px 彩虹线
        rainbow = ctk.CTkFrame(h, fg_color="transparent", height=3)
        rainbow.grid(row=1, column=0, sticky="ew")
        rainbow.grid_propagate(False)
        n = len(RAINBOW_COLORS)
        for i, c in enumerate(RAINBOW_COLORS):
            seg = tk.Frame(rainbow, bg=c, height=3, bd=0, highlightthickness=0)
    ...
    _DISPLAY_ONLY = {"show_grid", "show_board_lines", "show_color_codes"}

    def _on_param_changed(self, key: str, value):
        self.controller.set_param(key, value)
        if key in self._DISPLAY_ONLY:
            self._rerender()
        else:
            self._regenerate()
    ...
    def _on_zoom_changed(self, tile_size: int):
        ...
        if self.controller.current_pattern:
    ...
    def _regenerate(self):
        ...
        try:
            ...
            self.control_panel.set_bead_w_display(
                self.controller.get_params()["max_beads_w"])
        except Exception as e:
